...
STRICT RULES:
- Match the exact school-test style (clear headings + spacing).
- Use ONE item per line.
- Use clear line breaks.
- DO NOT use symbols (no ■, •, ▪, emojis, or decorative separators).
- Keep the A list and B list as separate blocks (no tables).
...
2- I love chocolate cake, but I hate vanilla.
3- Would you like to play outside?
4- My favourite movie is Toy Story because it’s funny!

B:
a- Yes, I would! Playing outside is fun.
b- My favourite animal is a dolphin because they are smart.
c- I think chocolate cake is the best dessert!
d- I prefer chocolate, too!
...
def main():
    ...
    def _clean_symbols(match: re.Match) -> str:
        s = match.group(0)
        return "\n\n" if len(s) >= 3 else " "

    t = re.sub(r"[■▪•]{1,}", _clean_symbols, t)

    # Remove any leftover non-printable control chars (keep tabs/newlines)
    t = re.sub(r"[^\x09\x0A\x20-\x7E]", "", t)

    # Ensure section headings look like Word tests
    t = t.replace("LANGUAGE FUNCTIONS:", "\nLANGUAGE FUNCTIONS:\n")
    t = t.replace("READING COMPREHENSION:", "\nREADING COMPREHENSION:\n")
    t = t.replace("VOCABULARY:", "\nVOCABULARY:\n")
    t = t.replace("GRAMMAR:", "\nGRAMMAR:\n")
    t = t.replace("WRITING:", "\nWRITING:\n")
    t = t.replace("ANSWER KEY:", "\nANSWER KEY:\n")

    # Ensure A/B are on their own lines
    t = re.sub(r"\bA:\s*", "\nA:\n", t)
    t = re.sub(r"\bB:\s*", "\nB:\n", t)
